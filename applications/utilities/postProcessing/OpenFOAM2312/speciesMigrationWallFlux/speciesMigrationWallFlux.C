...
void Foam::functionObjects::speciesMigrationWallFlux::writeFileHeader(Ostream& os) const//void Foam::functionObjects::speciesMigrationWallFlux::writeFileHeader(const label i)
{
    ...
    writeTabbed(os, "min");//writeTabbed(file(), "min");
    writeTabbed(os, "max");//writeTabbed(file(), "max");
    writeTabbed(os, "integral");//writeTabbed(file(), "integral");
    os << endl;//file() << endl;
}
...
bool Foam::functionObjects::speciesMigrationWallFlux::write()
{
    ...
    const volScalarField& speciesMigrationWallFlux = lookupObject<volScalarField>(type());

    Log << type() << " " << name() << " write:" << nl
        << "    writing field " << speciesMigrationWallFlux.name() << endl;

    speciesMigrationWallFlux.write();
